      ...
      this.sqlite.create({
        ...
        db.executeSql('create table if not exists taskExamine( id varchar(36) PRIMARY KEY not null,taskId int,name nvarchar(50) not null,desc nvarchar(500),seq int,examineOption int,creationTime DateTime);', [])
          .then(() => {
            ...
            db.executeSql('create table if not exists grower(id varchar(36) PRIMARY KEY not null,year int,unitCode nvarchar(20),unitName nvarchar(50),name nvarchar(50) not null,countyCode int,employeeId nvarchar(50),contractNo nvarchar(50),villageGroup nvarchar(50),tel nvarchar(20),address nvarchar(500),type int,plantingArea decimal(18,2),longitude decimal(18,6),latitude decimal(18,6),isEnable int,collectNum int not null,employeeName nvarchar(200),areaCode int,areaScheduleDetailId varchar(36),contractTime DateTime,unitVolume decimal(18,2),actualArea decimal(18,2),areaStatus int,areaTime DateTime);', [])
          }).then(() => {
            ...
            db.executeSql('create table if not exists scheduleDetail(id varchar(36) PRIMARY KEY not null,taskId int not null,scheduleId varchar(36) not null,employeeId nvarchar(50) not null,growerId int not null,visitNum int,completeNum int,status int,scheduleTaskId varchar(36) not null,employeeName nvarchar(50),growerName nvarchar(50),creationTime DateTime,areaTime DateTime, isUpload int);', [])
          }).then(() => {
            db.executeSql('create table if not exists visitRecord(id varchar(36) PRIMARY KEY not null,scheduleDetailId varchar(36) not null,employeeId nvarchar(50),growerId int,signTime DateTime,location nvarchar(200),longitude decimal(18,2),latitude decimal(18,2),desc nvarchar(500),imgPath nvarchar(2000),creationTime DateTime,isOnline int);', [])
          }).then(() => {
            db.executeSql('create table if not exists visitExamine(id varchar(36) PRIMARY KEY not null,visitRecordId varchar(36),employeeId nvarchar(50),growerId int,taskExamineId int,score int,creationTime DateTime);', [])
          }).then(() => {
            db.executeSql('create table if not exists growerAreaRecords(id varchar(36) PRIMARY KEY not null,growerId int not null,scheduleDetailId varchar(36) not null,imgPath nvarchar(2000),longitude decimal(11,8),latitude decimal(11,8),location nvarchar(200),employeeName nvarchar(200),employeeId nvarchar(50),collectionTime DateTime,area decimal(18,2),remark nvarchar(200),isOnline int);', [])
          }).then(() => {
